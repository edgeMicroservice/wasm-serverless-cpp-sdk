...
void mk_module_api_request(uint32_t memAddr, uint32_t len);
...
extern "C" {
  ...
  int32_t WASM_EXPORT_AS("mk_module_on_api_response") mk_module_on_api_response(int64_t sid, int64_t buf,
                                              uint32_t len);
  void WASM_EXPORT_AS("mk_module_abi_version_1_0_0") mk_module_abi_version_1_0_0();                         
}

namespace mimik {
namespace wasm {
using rpc_callback = std::function<int(const std::string &)>;

class rpc {
 public:
  static int32_t register_callback(rpc_callback callback);
  static void call(const std::string &json);
};
